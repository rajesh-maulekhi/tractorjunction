...
class Search_request_result extends CI_Controller
{
...
// echo $value_array;
        ...
        $ci->load->database();
        $ci->db->select('*');
        $ci->db->from('demo_request');
        $ci->db->where('date >=', $newDate2);
        $ci->db->where('date <=', $newDate1);
        $query = $ci->db->get();
        $qq1a = $query->result();
...
// print_r($qq1a);
        ...
        foreach ($qq1a as $result => $obj) {
            ...
            <tr>
                ...
                <td>
                    <?php echo form_open('admins/Reply_request'); ?>
