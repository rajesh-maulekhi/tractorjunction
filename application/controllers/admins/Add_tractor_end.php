...
class Add_tractor_end extends CI_Controller
{
    ...
    function index()
    {
        ...
        if ($_FILES['model_image']['name'] != "") {
            ...
            foreach (shweta_select('name', 'brand', 'id', $this->input->post('brand')) as $raa) $brand_name = shweta_nameslug($raa->name);
            $model_name = shweta_nameslug($this->input->post('model_name'));
            // $rand_no=rand(9,99999999999); 
            ...
            $config['file_name'] = newslugend($brand) . '-' . newslugend($this->input->post('model_name'));
            ...
            $image_name = "tractor_default.png";

        }

//slug
        $slug = '';
        $slug = SlugCreate('tech_specification', $this->input->post('model_name'));
