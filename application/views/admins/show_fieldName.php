<?php
$root = "http://" . $_SERVER['HTTP_HOST'];
$root .= str_replace(basename($_SERVER['SCRIPT_NAME']), "", $_SERVER['SCRIPT_NAME']);

if ($this->session->userdata('admin')) {
    ?>

    <title>Admin || Show Fields </title>

    <div id="loder" style="background:rgba(0, 0, 0, 0.55);height:100%;width:100%;display:none;position: fixed;
    left: 0;
    right: 0;
    top: 0;
    z-index: 2;">
        <center>
            <img src="<?php echo $root; ?>images/gif_tractor.gif" style="margin-top:23%;height:50px;">
        </center>
    </div>

    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12"
         style="padding:0px;min-height:835px;background: #F7F7F7;">

    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 " style="padding:20px 14px;">
        <div class="col-sm-2 col-md-2">
            <a href="<?php echo $root; ?>admins/add-fields">
                <button type="button" class="btn btn-default buttoupdate" style="" id="delete">
                    Add Fields
                </button>
            </a>
        </div>


    </div>


    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 "
         style="padding:20px 14px;background: #fff; margin-top:20px;">

        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 " style="padding:0px; padding-top:0px;">
            <h3>List Fields</h3>

            <hr style="  border-top: 3px solid #C5C2C2;">
            <?php
            $result = array();
            $condition = '';
            $result = shweta_pagination_query_new_orderby('implement_fileds', '9', 'admins/show-fields', $condition, 'id', 'DESC');
            // echo "<pre>";
            // print_r($result);
            // die;

            if (empty($result['result'])) {
                ?>
                <h5 style="    text-align: center;
    color: #DC4344;
    font-size: 22px;
    padding: 40px;">No result found</h5>
            <?php } else { ?>
                <table class="table table-striped">
                    <thead>
                    <tr>

                        <th style="width:5%;">S.no.</th>
                        <th style="width:25%;">Implement Name</th>
                        <th style="width:25%;">No of fields</th>

                        <th>Action</th>

                    </tr>
                    </thead>
                    <tbody>

                    <?php
                    $i = 1;
                    foreach ($result['result'] as $key => $value) {
                        ?>
                        <tr>

                            <td><?php echo $i; ?></td>
                            <td><?php

                                foreach (shweta_select('name', 'filed', 'id', $value->title) as $raa) echo ucfirst($raa->name);

                                ?></td>
                            <td><?php

                                $featurename = array();
                                $featurename_filter = array();
                                $featurename_count = "";

                                $featurename = explode('$$', $value->fieldsName);
                                $featurename_filter = (array_filter($featurename));
                                echo $featurename_count = count($featurename_filter);


                                ?></td>

                            <td>
                                <a href="<?php echo $root; ?>admins/edit-field-name/<?php echo $value->id; ?>">
                                    <button type="button" style="background:#1ABB9C;color:#eee;"
                                            class="ap btn btn-default"><i class="fa fa-edit"></i></button>
                                </a>
                                <a href="<?php echo $root; ?>admins/ImplementTractor/deleteallfiledNamessingle/<?php echo $value->id; ?>/<?php echo $value->title; ?>">
                                    <button style="background:#D43F3A;color:#eee;" type="button"
                                            class="ap btn btn-default"><i class="fa fa-trash"></i></button>
                                </a>


                            </td>
                        </tr>

                        <?php $i++;
                    }
                    ?>


                    </tbody>

                </table>
                <div class="pagination" style="float:right; margin-top: 13px;">
                    <ul class="pagination" style="margin:-4px 0px -23px 0px !important;">
                        <?php echo $result['links']; ?>
                    </ul>
                </div>
            <?php } ?>

        </div>
    </div>
    <script src="<?php echo $root; ?>/js/jquery.min2.js"></script>

    <script type="text/javascript">
        $(document).ready(function () {
            $('#select_all').on('click', function () {
                if (this.checked) {
                    $('.checkbox').each(function () {
                        this.checked = true;
                    });
                } else {
                    $('.checkbox').each(function () {
                        this.checked = false;
                    });
                }
            });

            $('.checkbox').on('click', function () {
                if ($('.checkbox:checked').length == $('.checkbox').length) {
                    $('#select_all').prop('checked', true);
                } else {
                    $('#select_all').prop('checked', false);
                }
            });
        });

        $(document).ready(function () {
            $('#deleteMultiple').on('click', function () {
                var vals = new Array();
                var chkArray12 = [];
                $(".checkbox:checked").each(function () {
                    chkArray12.push($(this).val());
                });
                if (chkArray12 != "") {
                    document.getElementById('loder').style.display = "block";
                    $.ajax({
                        type: "POST",
                        url: '../admins/ImplementTractor/deleteallfiledsName',
                        data: {val1: chkArray12},
                        success: function (data) {
                            document.getElementById('loder').style.display = "none";
                            alert(data);
                            location.reload();
                        },
                    });
                }
                else {
                    alert("please check atleast one Filed");
                }
            });
        });


    </script>
    <?php
} else {
    header("Location:" . $root . "admin");
}
?>