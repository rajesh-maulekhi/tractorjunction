...
if ($this->session->userdata('admin')) {
    ...
    <title>Admin || Show Service Center </title>
    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 " style="padding:20px 14px;">
        <div class="col-sm-2 col-md-2">
            <a href="<?php echo $root; ?>admins/add-service-center">
                <button type="button" class="btn btn-default buttoupdate" style="" id="delete">Add Service Center
                </button>
            </a>
        </div>
    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 "
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 " style="padding:0px; padding-top:0px;">
            <h3>Show Service Center</h3>

            <hr style="  border-top: 3px solid #C5C2C2;">
            <?php
            $result = array();
            $condition = '';
            $result = shweta_pagination_query_new_orderby('service_center', '9', 'admins/show-service-center', $condition, 'name', 'ASC');
            // echo "<pre>";
            // print_r($result);
            // die;
    ...
    color: #139ED9 !important;" id="select_all"></th>
                        ...
                        <th style="width:15%;">Service center brand</th>
                        <th style="width:15%;">Service center state</th>
                        <th style="width:15%;">Service center city</th>
                        <th>Action</th>
    ...
    color: #139ED9 !important;" class="checkbox" value="<?php echo $value->id; ?>"/></td>
                            ...
                            <td><?php foreach (shweta_select('name', 'states', 'id', $value->state) as $raa) echo $raa->name ?></td>
                            <td><?php foreach (shweta_select('name', 'cities', 'id', $value->city) as $raa) echo $raa->name ?></td>

                            <td>
                                <a href="<?php echo $root; ?>admins/edit-service-center/<?php echo $value->id; ?>">
                                    <button type="button" style="background:#1ABB9C;color:#eee;"
                                            class="ap btn btn-default"><i class="fa fa-edit"></i></button>
                                </a>
                                <a href="<?php echo $root; ?>admins/ServiceCenter/DeleteServiceCenter?id=<?php echo $value->id; ?>">
        ...
        $(document).ready(function () {
            $('#deleteMultiple').on('click', function () {
                ...
                if (chkArray12 != "") {
                    ...
                    $.ajax({
                        type: "POST",
                        url: '../admins/ServiceCenter/delallservices',
                    ...
                    alert("please check one service");
                }
            });
        });
