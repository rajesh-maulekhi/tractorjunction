...
if ($this->session->userdata('admin')) {
    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 " style="padding:20px 14px;">
        <div class="col-sm-2 col-md-2">
            <a href="<?php echo $root; ?>admins/news-update">
                <button type="button" class="btn btn-default buttoupdate" style="" id="delete">Add News</button>
            </a>
        </div>
    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 "
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 " style="padding:0px; padding-top:0px;">
            ...
            $result = array();
            $condition = '';
            $result = shweta_pagination_query_new_orderby('news', '9', 'admins/show-news-update', $condition, 'id', 'DESC');
            // echo "<pre>";
            // print_r($result);
            // die;
    ...
    color: #139ED9 !important;" class="checkbox" value="<?php echo $value->id; ?>"/></td>
                            ...
                            <td><?php
                                if (strlen($value->title) > 50) {
                                    echo substr(ucwords(strtolower($value->title)), 0, 50) . "...";
                                } else {
                                    echo ucwords(strtolower($value->title));
                                }
                                ?></td>
                            <td><?php
                                if (strlen($value->description) > 50) {
                                    echo substr(ucwords(strtolower($value->description)), 0, 50) . "...";
                                } else {
                                    echo ucwords(strtolower($value->description));
                                }
                                ?></td>


                            <td>
                                <a href="<?php echo $root; ?>admins/edit-news/<?php echo $value->id; ?>">
                                    <button type="button" style="background:#1ABB9C;color:#eee;"
                                            class="ap btn btn-default"><i class="fa fa-edit"></i></button>
                                </a>
                                <a href="<?php echo $root; ?>admins/NewsAndUpdate/DeleteNews?id=<?php echo $value->id; ?>">
        ...
        $(document).ready(function () {
            $('#deleteMultiple').on('click', function () {
                ...
                if (chkArray12 != "") {
                    ...
                    $.ajax({
                        type: "POST",
                        url: '../admins/NewsAndUpdate/DeleteNewsAll',
